...
* Genuine asynchrony using the AWS REST APIs and aiohttp
...
In `app.py` we'll have the main code to run the comparisons between the approaches, then we'll have a data package which will contain a nice helper to generate test data to write to DynamoDB, as well as a dynamodb package which will contain the functionality to perform the various type of writes we're interested in along with any useful helper functions.
...
Here we have a little dataclass to hold a given book's data and a function that, given a `Faker`, returns an instance of our `Book` dataclass with some randomly generated attributes.
...
Our `write_records` function is a simple as it gets; it takes a list of books and the name of a DynamoDB table, then puts each of them to DynamoDB in a for loop (we're not bothering with any error handling or retry logic here). The function implementing the core functionality, `boto3_put_book_to_dynamodb`, is a helper function that looks like the following:

{{< gist Jamie3213 0851c3ad61dadcb2e229f1d0066f2f03 >}}

This function simply calls the `PutItem` API using Boto3 and formats the attributes we want to write to DynamoDB in the correct way (the client wrapper is a fairly thin abstraction over the actual REST API, which makes things a little easier later on).
...
Now, using the aiohttp library is broadly the same as using the requests library, so why do I keep implying that I have a strong aversion to using this method in the first place? The answer is that whilst *calling* the APIs in a non-blocking way is easy, constructing the headers for the low-level AWS REST APIs is a convoluted nightmare. In AWS's defense, there are hints like this in all of their (very limited) documentation of the low-level APIs suggesting that you should probably just stick with the official SDKs instead of hacking together the headers yourself:

!["AWS warning that you should stick to the SDKs"](/images/dynamodb_concurrency/aws_warning.png)

Alas, we're going to ignore their suggestion. Any calls made to low-level AWS REST APIs need to use what AWS calls the [*signature version 4 signing process*](https://docs.aws.amazon.com/general/latest/gr/signature-version-4.html) which is essentially a very specific set of information, formatted in a very specific way that's passed along with every request. AWS [describe the process](https://docs.aws.amazon.com/general/latest/gr/sigv4_signing.html) like this:

* Step 1: [create a canonical request for Signature Version 4](https://docs.aws.amazon.com/general/latest/gr/sigv4-create-canonical-request.html)
* Step 2: [create a string to sign for Signature Version 4](https://docs.aws.amazon.com/general/latest/gr/sigv4-create-string-to-sign.html)
* Step 3: [calculate the signature for Signature Version 4](https://docs.aws.amazon.com/general/latest/gr/sigv4-calculate-signature.html)
...
We've defined three tables, SyncBooks, ThreadedBooks and AsyncBooks (one for each of the approaches), and also set the number of books to generate to 10,000. In addition, we've also set a seed for our `Faker` class to ensure that we write the same data to each table to make the comparison fair.
...
With that done, we can run our Python script and see how our different approaches stack up against one another:

```zsh
python src/app.py
```

Once the script completes, we get the following results:

!["Bar chart showing a comparison of execution times for each approach"](/images/dynamodb_concurrency/comparison.png)
